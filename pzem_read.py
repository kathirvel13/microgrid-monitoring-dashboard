...
# MQTT CONFIG
# -----------------------------
MQTT_BROKER = "broker.emqx.io"   # CHANGE THIS
MQTT_PORT = 1883
...
# MQTT PUBLISH
# -----------------------------
def publish_mqtt(source, voltage, current, power, frequency, pf):
...
while True:
    ...
    sources = {
        ...
        27:{"voltage":v2,"current":i2,"power":p2, "frequency":f2, "power factor": pf2},
        28:{"voltage":v3,"current":i3,"power":p3, "frequency":f3, "power factor" : pf3}
    }
    ...
    # MQTT PUBLISH
    publish_mqtt(1,v1,i1,p1,0,0)          # DC
    publish_mqtt(2,v2,i2,p2,f2,pf2)       # AC
    publish_mqtt(3,v3,i3,p3,f3,pf3)       # AC
    ...
    for slave,data in sources.items():
        ...
        if voltage < VOLTAGE_THRESHOLD and not source_failed[slave]:
            ...
            print(f"Switched {slave} â†’ Relay {relay_to_use}")
